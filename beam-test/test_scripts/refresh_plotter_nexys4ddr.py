...
def run_bist_cmds(args, refresh_rate_in : int, wait_time : int, serial_fdspawn : fdspawn, with_ones : bool, err_list_file, ones_zeros):

    # First set the refresh rate
    serial_fdspawn.sendline(BIST_REFRESH_CMD_SR.format(refresh_rate=refresh_rate_in))
    ...
    pattern_fdspawn = [BIST_ERROR_MSG_REGEX, pexpect.TIMEOUT]
    fdspawn_index = serial_fdspawn.expect(pattern_fdspawn, timeout=120)
    ...
    if (fdspawn_index == 0):
        ...
        # err_list_file.write(err_point_str)

        # serial_fdspawn.expect(LITEX_LOGIN_PATTERN, timeout=None)
        
        while(True):
            try:
                serial_fdspawn.expect(BIST_ERROR_MSG_REGEX, timeout=30)
                new_err_str = re.search(pattern=BIST_ERROR_MSG_REGEX, string=serial_fdspawn.match.group(0))
                if new_err_str != None:
                    pass
                    # err_list.append(new_err_str.group(0))
                else:
                    print("No match!")
                

            except pexpect.exceptions.TIMEOUT:
                print("Left after exception!!")
                break
    ...
    serial_fdspawn.sendline(BIST_NEWLINE_CHARS)
    time.sleep(1)
    serial_fdspawn.expect(LITEX_LOGIN_PATTERN)

    # Return match object 
    return serial_fdspawn, err_count
...
def run_test_get_results(args, err_list_file, serial_fdspawn:fdspawn, refresh_rate:int, wait_time:int, with_ones:bool):
    ...
    serial_fdspawn, err_count = run_bist_cmds(
        args=args,
        refresh_rate_in=refresh_rate,
        wait_time=wait_time,
        serial_fdspawn=serial_fdspawn,
        with_ones=with_ones,
        err_list_file = err_list_file,
        ones_zeros = ones_zeros)
...
def main():
    ...
    if args.log_dir:
        ...
        # zeros_err_list_file = open(args.log_dir + '/' + args.filename + '_WRITING_ZEROS.log', 'w')
        # ones_err_list_file = open(args.log_dir + '/' + args.filename + '_WRITING_ONES.log', 'w')
    else:
        pass
        # # Create files to hold all errors output
        # zeros_err_list_file = open(args.filename + '_WRITING_ZEROS.log', 'w')
        # ones_err_list_file = open(args.filename + '_WRITING_ONES.log', 'w')
