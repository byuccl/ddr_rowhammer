...
STARTING_PAGE_NUM = 1
STARTING_DECREMENTING_RANK_NUM = 17
NUM_RANKS = 17 # Ranks range from index 0 - 16, 17 means nonexistentrank
...
def main():
    ...
    print(refresh_test_ranks)
    file_desc.write(str(refresh_test_ranks))
                            ...
                            for line_num, line in enumerate(openedFile, STARTING_PAGE_NUM):
                                ...
                                if bist_matching_str in line:
                                    ...
                                    if ((len(dataval[0]) != 8) or (len(dataval[1]) != 8)):
                                        print("Skipping dataval: ", dataval)
                                        file_desc.write("Skipping dataval: " + dataval)
                                        continue
    ...
    file_desc.write("\n\n\n\n\n\n\n\nRank map {bank, row, col, bit_num, rank), rank:0-16, 17 means nonexistant (17 refresh tests):\n")
    file_desc.write("\n".join(["[" + str(key) + " : " + str(value) + "]" for key, value in rank_map.items()]))

    # Create a list of tuples from information
    print("Creating tuple list (bank, row, col, bit_num, freq, rank), rank:0-16, 17 means nonexistant (17 refresh tests):")
    file_desc.write("Creating tuple list (bank, row, col, bit_num, freq, rank), rank:0-16, 17 means nonexistant (17 refresh tests):")
    organized_rank_list = []
    ...
    file_desc.write("\n\n\n\n\n\n\n\nOrganized list (bank, row, col, bit_num, freq, rank), rank:0-16, 17 means nonexistant (17 refresh tests):\n")
