...
def main():
    ...
    # with open("./nexys4ddr_complete_characterization_nonirradiated/nexys4ddr_complete_characterization_nonirradiated_organized.log") as openedFile:
        for num, line in enumerate(openedFile, 1):
            ...
            if (re.search(REGEX_ERROR_STR, line) != None):
                ...
                bankString = timeString[4] # Address is 7 digits including a 0 to the left, row address is bits 0 - 12, bank is bits 13 - 16 so get the 
                                           # letter corresponding to bits 12 - 16.
                bankInt = int(bankString, base=16) # Get the integer of this, subtract high bit if needed (value of bank is between 0 - 7)
                assert ((bankInt >= 0) and (bankInt <= 15))
                if (bankInt >= 8): 
                    bankInt -= 8
                
                # print("TimeString = '", timeString, "', len: ", len(timeString), ", bank str: ", bankString, ", bank int: ", bankInt)
    ...
    # Hope this works for bank 0 bit flips
