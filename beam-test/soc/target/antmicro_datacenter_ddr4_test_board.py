    ...
    def __init__(self, *, sys_clk_freq=100e6, iodelay_clk_freq=200e6,
        ...
        if not self.integrated_main_ram_size:
            ...
            if with_ecc_debug:
                ...
                # Added imports
                #####################################################
                from litedram.frontend.ecc_test import DMA_Reader, DMA_Writer
                from litedram.frontend.ecc import LiteDRAMNativePortECC
                from litedram.common import LiteDRAMNativePort
                #####################################################

                ###################################################################
                # Added code for ecc test
                ###################################################################
                # Get all four ports
                nonecc_w = self.sdram.crossbar.get_port()
                nonecc_r = self.sdram.crossbar.get_port()
                ecc_helper_w = self.sdram.crossbar.get_port()
                ecc_helper_r = self.sdram.crossbar.get_port()
                ...
                # Add ECC Modules
                self.submodules.sdram_eccr = LiteDRAMNativePortECC(
                    ecc_r, ecc_helper_r,
                    with_error_injection=False
                )
                self.submodules.sdram_eccw = LiteDRAMNativePortECC(
                    ecc_w, ecc_helper_w,
                    with_error_injection=True
                )

                # Finally, add all modules to control ports
                setattr(self.submodules, f"non_ecc_writer", DMA_Writer(nonecc_w))
                setattr(self.submodules, f"non_ecc_reader", DMA_Reader(nonecc_r))
                setattr(self.submodules, f"ecc_writer", DMA_Writer(ecc_w))
                setattr(self.submodules, f"ecc_reader", DMA_Reader(ecc_r))
                self.add_csr("non_ecc_writer")
                self.add_csr("non_ecc_reader")
                self.add_csr("ecc_writer")
                self.add_csr("ecc_reader")
                # print("ECC data width: ", ecc_r.data_width)
                # print("NON ECC data width: ", nonecc_r.data_width)
                # print("ECC address width: ", ecc_r.address_width)
                # print("NON ECC address width: ", nonecc_r.address_width)
                ###################################################################
...
def main():
    ...
    parser.add_target_argument("--with-ecc-debug",         action="store_true",    help="Add ecc debug functions.")
    ...
    soc = BaseSoC(
        ...
        with_ecc_debug         = args.with_ecc_debug,
        **parser.soc_argdict)
